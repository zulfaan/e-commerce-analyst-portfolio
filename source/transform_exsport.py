...
from func import extract_color, generate_product_id, generate_color_id, generate_category_id
import pandas as pd
import luigi
import os
...
class TransformTokpedExportData(luigi.Task):
    ...
    def run(self):
        ...
        # Ekstrak nama produk
        df_name = {
            'product_id' : [generate_product_id(i) for i in range(len(df_product['name_product'].unique()))],
            'name_product' : df_product['name_product'].str.replace('"', '').str.split('-').str[0].str.upper().unique()
        }
